...
```
+ 사용자가 수정하고 싶은 데이터가 글 양식에 채워져 있도록
```
print('''<!doctype html>
<html>
<head>
...
```
+ 사용자가 title 수정할 수 없도록
+ input type을 `text`가 아니라 `hidden`으로 하자.
+ create_py가 아니라 update_py로 전송되어야 하니까 update_py로 변경.
```
print('''<!doctype html>
<html>
<head>
